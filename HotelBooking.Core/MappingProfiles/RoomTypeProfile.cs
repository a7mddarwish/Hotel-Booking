    ...
    internal class RoomTypeProfile : Profile
    {
        public RoomTypeProfile()
        {
            ...
            CreateMap<RoomType, ShowRoomTypeDTO>()
                ...
                //.ForMember(dest => dest.amenities, opt => opt.MapFrom(src => src.Amenities.Select(a =>new AmenitieyDTO {Name = a.Name , imagedto = new DisplayImageDTO {ImageUrl = a.ImageInfo.DisplayUrl 
                //, ImageID = a.ImageInfo.ImageID} })))
                .ForMember(dest => dest.Name, opt => opt.MapFrom(src => src.Name))
                .ForMember(dest => dest.Capacity, opt => opt.MapFrom(src => src.Capacity))
                .ForMember(dest => dest.PricePerNight, opt => opt.MapFrom(src => src.PricePerNight))
                .ForMember(dest => dest.Description, opt => opt.MapFrom(src => src.Description));
